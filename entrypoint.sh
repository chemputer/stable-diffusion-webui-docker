...
# Define the download marker path
DOWNLOAD_MARKER="/data/.download_complete"

# Check if the download process has been completed
if [ ! -f "$DOWNLOAD_MARKER" ]; then
  echo "Running download service..."
  /docker/download.sh

  # Mark the download as complete
  touch "$DOWNLOAD_MARKER"
else
  echo "Download already completed. Skipping."
fi

# Proceed with the original entrypoint logic
...
shopt -s nullglob
# For install.py, refer to the WebUI wiki for extensions
list=(./extensions/*/install.py)
for installscript in "${list[@]}"; do
  EXTNAME=$(echo $installscript | cut -d '/' -f 3)
  # Skip installing dependencies if the extension is disabled in config
